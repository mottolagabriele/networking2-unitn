...
do
  ...
  # # slice 2
  # # h0 -> s0, s2, s4
    sudo ovs-ofctl add-flow ${device} ip,priority=65500,nw_src=${ip_h0},nw_dst=${ip_s0},idle_timeout=0,actions=set_queue:1,normal
    sudo ovs-ofctl add-flow ${device} ip,priority=65500,nw_src=${ip_h0},nw_dst=${ip_s2},idle_timeout=0,actions=set_queue:1,normal
    sudo ovs-ofctl add-flow ${device} ip,priority=65500,nw_src=${ip_h0},nw_dst=${ip_s4},idle_timeout=0,actions=set_queue:1,normal

  # # s0, s2, s4 -> h0
    sudo ovs-ofctl add-flow ${device} ip,priority=65500,nw_src=${ip_s0},nw_dst=${ip_h0},idle_timeout=0,actions=set_queue:1,normal
    sudo ovs-ofctl add-flow ${device} ip,priority=65500,nw_src=${ip_s2},nw_dst=${ip_h0},idle_timeout=0,actions=set_queue:1,normal
    sudo ovs-ofctl add-flow ${device} ip,priority=65500,nw_src=${ip_s4},nw_dst=${ip_h0},idle_timeout=0,actions=set_queue:1,normal

    # Aggiungi queste righe al tuo script
    sudo ovs-ofctl add-flow ${device} ip,priority=65500,nw_src=${ip_h0},nw_dst=${ip_s0},idle_timeout=0,actions=set_queue:1,normal
    sudo ovs-ofctl add-flow ${device} ip,priority=65500,nw_src=${ip_s0},nw_dst=${ip_h0},idle_timeout=0,actions=set_queue:1,normal


    #sudo ovs-ofctl add-flow ${device} priority=65500,actions=normal
