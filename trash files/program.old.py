...
class NetworkApp(tk.Tk):
    ...
    def start_my_network(self):
          # Avvia il processo my_network.py e ottiene la sua console
        self.my_network_process = subprocess.Popen(['sudo', 'python3', 'my_network.py'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.PIPE)
        # Leggi l'output della console di my_network.py e aggiungilo alla tua interfaccia grafica
        for line in self.my_network_process.stdout:
            self.console2.insert(tk.END, line.decode('utf-8'))

    def ping(self):
         # Invia il comando pingall alla console di my_network.py
        if hasattr(self, 'my_network_process') and self.my_network_process:
            self.my_network_process.stdin.write(b"pingall\n")
            self.my_network_process.stdin.flush()
